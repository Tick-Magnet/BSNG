...
void run_sng_algo_uf(ClusteringAlgo& sng) {	
    ...
    #pragma omp parallel private(root, root1, root2, thread_id, neighbors, neighbor_point_id, i, j, point_id) shared(pointQueues, sng, prID)
    {
        ...
        /*        #pragma omp critical
        {
            ...
            for (int seed_index = start_seed; seed_index < end_seed; ++seed_index) {
                cout << sng.selected_seeds[seed_index] << " ";
            }
            cout << endl;
        }
        */
        ...
        while (!pointQueue.empty()) {
			...
			*/
            ...
            if (neighbors.size() >= sng.m_minPts && sng.m_corepoint[currentPoint] != 1) {
                ...
                std::sort(neighbors.begin(), neighbors.end(), [](const kdtree2_result& a, const kdtree2_result& b) {
                    return a.idx > b.idx;
                });
